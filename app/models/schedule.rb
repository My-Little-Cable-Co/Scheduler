class Schedule
  def self.for_channel(channel, first_day, last_day)
      ...
      lineup: listings.map do |airtime, potentially_overlapped_listings|
        ...
        if(listing.parent_listing_id.nil?)
          ...
          end
          {
            ...
            # NOTE: This file selection should be considered temporary. We
  ...
  def self.for(first_day, last_day)
    ...
    Channel.order(:number).each do |channel|
      schedule[:channels] << for_channel(channel, first_day, last_day)
    end
